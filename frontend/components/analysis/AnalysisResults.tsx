...
const AnalysisResults: React.FC<AnalysisResultsProps> = ({ analysis, className = '' }) => {
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      {Object.keys(analysis.agents || {}).length > 0 && (
        <div className="bg-slate-800 rounded-lg p-6">
          <h2 className="text-xl font-bold mb-4 text-slate-200">Multi-Agent Analysis Workflow</h2>
          <p className="text-sm text-slate-400 mb-6">Comprehensive analysis from specialized AI agents working in parallel</p>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(analysis.agents).map(([agentId, agent]) => {
              // Special handling for committee simulator
              const isCommitteeAgent = agentId === 'committee_simulator';
              const committeeData = isCommitteeAgent ? agent.data : null;

              return (
                <div key={agentId} className={`rounded-lg p-4 border ${
                  isCommitteeAgent
                    ? 'bg-gradient-to-br from-purple-900/20 to-blue-900/20 border-purple-500/50'
                  ...
                  <div className="flex items-center justify-between mb-3">
                      ...
                      {isCommitteeAgent ? (
                        <>
                          <svg className="w-4 h-4 mr-2 text-purple-400" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <path stroke="#a855f7" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18 8a6 6 0 01-12 0" />
                          </svg>
                    ...
                    {hasConfidence(agent) && agent.confidence !== undefined && (
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        agent.confidence >= 0.8 ? 'bg-green-900/30 text-green-400' :
                        agent.confidence >= 0.6 ? 'bg-yellow-900/30 text-yellow-400' :
                  ...
                  {isCommitteeAgent && committeeData ? (
                    <div className="space-y-3">
                      <div className="bg-slate-800/50 p-3 rounded border border-slate-600">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs text-slate-400 uppercase tracking-wide">Committee Verdict</span>
                          <span className={`px-2 py-1 rounded text-xs font-bold border ${
                            committeeData.committee_verdict === 'INVEST' ? 'bg-green-900/30 text-green-400 border-green-500' :
                            committeeData.committee_verdict === 'CONSIDER' ? 'bg-yellow-900/30 text-yellow-400 border-yellow-500' :
                            'bg-red-900/30 text-red-400 border-red-500'
                          }`}>
                            {committeeData.committee_verdict}
                          </span>
                        </div>
                          ...
                          <div className="w-16 bg-slate-700 rounded-full h-1.5">
                            <div
                              className={`h-1.5 rounded-full transition-all duration-500 ${
                                committeeData.consensus_score >= 0.8 ? 'bg-green-500' :
                                committeeData.consensus_score >= 0.6 ? 'bg-yellow-500' : 'bg-red-500'
                              }`}
                        ...
                        <p className="text-xs text-slate-400">
                          {committeeData.member_count} committee members participated
                        </p>
                      </div>
                      <p className="text-xs text-slate-300 italic">
                        {agent.data?.summary || 'Investment committee simulation completed'}
                      </p>
                    </div>
                  ) : hasError(agent) && agent.error ? (
                    <div className="text-red-400 text-sm p-2 bg-red-900/20 rounded">
                      Error: {agent.error}
                    </div>
                  ) : hasData(agent) ? (
                    <div className="text-sm text-slate-300 bg-slate-800/50 p-3 rounded max-h-32 overflow-y-auto">
